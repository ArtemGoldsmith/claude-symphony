...
| Preview driver | 🟡 Partial | `src/control-plane/{engine,daemon,process-manager}.ts` — dispatches `preview.up_script`/`down_script` as non-agent runs via the run-wrapper (`buildScriptEnv`, no Linear token); `previewing`/`tearing_down` reconcile via `exit.json`; `canPreview` guards `closing → previewing`; `preview.timeout_seconds` ceiling group-kills a hung run → `preview_failed`/`teardown_failed` (retryable, slot-conditional) |
